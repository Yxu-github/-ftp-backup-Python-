...
from progressbar import AnimatedMarker, Bar, BouncingBar, Counter, ETA, AdaptiveETA, FileTransferSpeed, FormatLabel, \
    Percentage, ProgressBar, ReverseBar, RotatingMarker, SimpleProgress, Timer, UnknownLength
...
class Xfer(object):
    ...
    def uploadFile(self, localpath, remotepath):
        ...
        self.debug_print("开始上传%s"%localpath)
    ...
    def __process_print(self, block):
        ...
        '''
        self.bar+=len(block)
        self.progressbar(self.bar*10/self.file_size,10)

    def __file_write(self, block):
        '''用于下载时文件写入和显示下载进度'''
        self.file.write(block)
        self.__process_print(block)
